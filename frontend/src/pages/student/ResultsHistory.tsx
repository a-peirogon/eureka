    ...
    <AppLayout>
      ...
      {completed.length > 0 && (
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            { value: completed.length, label: 'Completados', color: 'text-slate-900' },
            {
          ...
          {(attempts ?? []).map((a: any) => (
            <div key={a.id} className={clsx(
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-slate-900">{a.exam_title ?? 'Simulacro'}</p>
                ...
                {a.score_by_area && Object.keys(a.score_by_area).length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {Object.entries(a.score_by_area).map(([area, pct]) => (
                      <span key={area} className="text-[11px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full font-medium">
