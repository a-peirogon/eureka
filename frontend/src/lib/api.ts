...
export const analyticsApi = {
  ...
  // Returns personalized resources based on historical performance.
  // Useful for the Dashboard. The Results page derives recommendations
  // directly from the attempt's score_by_area (no extra request needed).
  recommendations: () =>
  api.get('/analytics/recommendations/me').then((r) => r.data),
  // Fire-and-forget: log that a student opened a resource.
  trackRecommendation: (data: {
    area: string
    plataforma: string
    url: string
    attempt_id?: string
  }) => api.post('/analytics/recommendations/track', data).catch(() => {}),
}
